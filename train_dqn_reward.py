...
Train a DQN agent on ALE/MsPacman-v5 using reward shaping.

The reward shaping formula is:

shaped_reward = original_reward - step_penalty_alpha - life_loss_penalty_beta * lives_lost_step

This script:
- reads an optional YAML configuration file;
- creates the vectorized Ms. Pac-Man environment;
- enables RewardShapingWrapper;
...
def train_dqn_with_reward_shaping(
        total_timesteps: int,
        seed: int,
        model_path: str,
        tensorboard_log: str,
        checkpoint_dir: str,
        step_penalty_alpha: float,
        life_loss_penalty_beta: float,
        learning_rate: float,
        buffer_size: int,
        learning_starts: int,
        batch_size: int,
    ...
    """
    Train a DQN model using reward shaping.
    """
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="dqn_reward_mspacman",
    ...
    model.learn(
        ...
        tb_log_name="dqn_reward_mspacman",
        progress_bar=True,
    )
    ...
    print(f"Reward-shaped DQN model saved to: {model_path}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train a reward-shaped DQN agent on ALE/MsPacman-v5."
    )

    parser.add_argument(
        "--config",
        type=str,
        default="config/dqn_reward.yaml",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path where the trained reward-shaped model will be saved. Overrides YAML config if provided.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Penalty applied at each agent step. Overrides YAML config if provided.",
    )

    parser.add_argument(
        "--life-loss-penalty-beta",
        type=float,
        default=None,
        help="Penalty applied when a life loss is detected. Overrides YAML config if provided.",
    )
...
if __name__ == "__main__":
    ...
    model_path = (
        ...
        if args.model_path is not None
            ...
            "paths",
            "model_path",
            "models/dqn_reward_mspacman.zip",
        )
    )
    ...
    step_penalty_alpha = (
        args.step_penalty_alpha
        if args.step_penalty_alpha is not None
        else get_config_value(config_data, "reward_shaping", "step_penalty_alpha", 0.01)
    )

    life_loss_penalty_beta = (
        args.life_loss_penalty_beta
        if args.life_loss_penalty_beta is not None
        else get_config_value(config_data, "reward_shaping", "life_loss_penalty_beta", 1.0)
    )

    train_dqn_with_reward_shaping(
        total_timesteps=total_timesteps,
        seed=seed,
